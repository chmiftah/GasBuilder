<script lang="ts">
	...
	import AppPreviewView from '$lib/components/AppPreviewView.svelte';
	...
	let activeTab = $state<'chat' | 'prd' | 'code' | 'app'>('chat');
	...
	onMount(() => {
		...
		if (project.files && project.files.length > 0) {
			...
			// If project already has ready app/HTML, show Tampilan Aplikasi directly when accessing http://localhost:5173/!
			if (project.files.some(f => f.type === 'HTML')) {
				activeTab = 'app';
			}
		}
	});
	...
	function handleSelectProject(selected: ProjectState) {
		...
		activeTab = selected.files.some(f => f.type === 'HTML') ? 'app' : selected.files.length > 0 ? 'code' : selected.prd ? 'prd' : 'chat';
	}
...
<div class="min-h-screen flex flex-col bg-[#070b14] text-slate-100">
	...
	<div class="flex-1 flex overflow-hidden">
		{#if activeTab === 'app'}
			<!-- Full-Width Interactive App View -->
			<section class="flex-1 h-[calc(100vh-4rem)] flex flex-col overflow-hidden">
				<AppPreviewView 
					files={project.files}
					prd={project.prd}
					onSwitchToCode={() => activeTab = 'code'}
					onSwitchToChat={() => activeTab = 'chat'}
					onDeploy={() => isDeployOpen = true}
				/>
			</section>
		{:else}
	...
	<GenerateProgressModal 
		...
		onClose={() => {
			...
			if (project.files.length > 0) activeTab = 'app';
		}}
	/>
</div>
